...
*(Note: n_stops is a scenario input label — not a model feature. The pre-race probabilities do not change when only the strategy label changes; they change when grid position or other pre-race features change.)*
