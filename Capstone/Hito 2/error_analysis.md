...
**Observation:** `is_top5` at semi-street circuits has the _lowest_ Brier (0.070) of any circuit type, which appears counterintuitive. The explanation is that top-5 finishes at semi-street circuits are heavily concentrated among front-tier constructors starting near the front; the model correctly assigns low P(top5) to the majority of the grid, making calibrated errors small even when the circuit is chaotic.
