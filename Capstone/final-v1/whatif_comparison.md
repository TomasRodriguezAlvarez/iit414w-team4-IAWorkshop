...
All probabilities produced with `SEED = 414` (RF `random_state=SEED`, LR `random_state=SEED`).
...
| ------------ | ---- | ---------------- | --------- | --------- |
| Front row    | P2   | n_stops=1, M-H   | **0.909** | **0.842** |
| Grid dropped | P7   | n_stops=1, M-H   | **0.810** | **0.541** |
| Grid dropped | P7   | n_stops=2, M-M-H | **0.810** | **0.541** |
...
| -------- | ----- | ----- | ------ | ------------- |
| P(top10) | 0.909 | 0.810 | −0.099 | −10.9%        |
| P(top5)  | 0.842 | 0.541 | −0.301 | −35.7%        |

The grid drop from P2 to P7 costs **10pp on P(top10)** but **30pp on P(top5)** — three times the relative damage at the tighter boundary.

### Why this matters for strategy

**If the team only models `is_top10`:**

> "Hamilton at P7 has an 81% chance of scoring points. That's high. 1-stop M-H is safe — no reason to take the extra pit-stop risk."

**After adding `is_top5`:**

> "Hamilton at P7 has only a 54% chance of reaching the top 5 — 30pp below the P2 baseline. The grid drop hasn't just shifted him slightly down the order; it has effectively removed him from the podium battle. **A 2-stop M-M-H is justified not to protect points-scoring (still 81%) but to recover track position for a top-5 challenge.** The marginal risk of an extra stop is worth taking given how much P(top5) was lost from starting P7."

This recommendation is structurally invisible from `is_top10` alone. The 81% P(top10) would be read as "safe." Only the P(top5) drop reveals that the driver is now off-pace for the tier of finish the team is targeting.
...
| ---------------------------------------------------- | ----------------------------- | --------------------------------------------------------------- |
| Should Hamilton attempt a 2-stop from P7?            | "No — points are safe at 81%" | "Yes — P(top5) dropped 30pp; 2-stop recovers the podium battle" |
